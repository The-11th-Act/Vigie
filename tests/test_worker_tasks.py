"""Tests de la tâche Celery d'ingestion.

C'est le code le moins couvert du projet (37 %) et le plus difficile à
déboguer en production : sans requête HTTP à laquelle rattacher une erreur, un
...
class TestScanTypeDispatch:
    ...
    def test_an_unknown_type_fails_immediately_without_retrying(self, fake_session):
    ...
    def test_the_scan_type_is_case_insensitive(self, fake_session):
        with patch.dict(PARSERS, {"nessus": lambda _: []}), run_task() as task:
            result = task("<xml/>", "NESSUS")
        assert result["status"] == "success"

    def test_a_missing_scan_type_is_handled(self, fake_session):
        with run_task() as task:
            result = task("<xml/>", None)
        assert result["status"] == "error"


class TestSuccessPath:
    def test_returns_the_ingestion_counters(self, fake_session):
        ...
        with (
            ...
            result = task(VALID_XML.decode(), "nessus")

        assert result["status"] == "success"
        assert result["processed_records"] == 7
        assert result["new_assets"] == 2

    def test_the_session_is_always_closed(self, fake_session):
        from app.services.ingestion import IngestionResult
...
class TestFailurePath:
    def test_a_failure_rolls_back_and_closes(self, fake_session):
    ...
    def test_the_exception_is_reraised_so_celery_can_retry(self, fake_session):
    ...
    def test_the_last_attempt_returns_an_error_instead_of_raising(self, fake_session):
