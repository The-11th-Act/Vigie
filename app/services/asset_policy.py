...
def criticality_for(ip_address: Optional[str]) -> Criticality:
    ...
    best_match: Optional[ipaddress._BaseNetwork] = None
...
def _as_criticality(level: Optional[str]) -> Criticality:
    if not level:
        return DEFAULT_CRITICALITY
    try:
