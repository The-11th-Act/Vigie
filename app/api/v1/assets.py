...
def get_assets(
    ...
    if criticality:
        query = query.filter(Asset.business_criticality == criticality.capitalize())
...
def create_asset(asset_in: AssetCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(status_code=409, detail="Asset with this IP address already exists")
...
def update_asset(asset_id: int, asset_in: AssetUpdate, db: Session = Depends(get_db)):
    ...
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
    for key, val in asset_in.model_dump(exclude_unset=True).items():
